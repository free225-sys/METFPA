...
import { AlertTriangle, Briefcase, CheckCircle2, ChevronDown, Clock, Edit3, Filter, History, Loader2, Lock, RefreshCw, Send, ShieldAlert } from "lucide-react";
...
export default function PlanAction() {
  ...
  return <div className="space-y-5 animate-slide-up" data-testid="page-plan-action">
    ...
    <header className="bg-white rounded-[8px] border border-[var(--border)] p-5">
      ...
      {/* Le périmètre doit être énoncé, pas deviné : l'agence ne voit que ses missions. */}
      {isDirection && (
        <p className="text-[13px] text-[var(--ink-500)] mt-2 flex items-start gap-1.5" data-testid="scope-notice">
          <Lock size={13} className="mt-[3px] shrink-0" aria-hidden="true" />
          <span>Vous voyez et mettez à jour uniquement les missions de votre agence (<strong>{user.direction}</strong>).</span>
        </p>
      )}
...
// Divulgation progressive : une section repliable n'impose pas ses champs à
// l'écran. Elle s'ouvre d'office quand elle contient déjà une information
// (blocage, arbitrage, livrable), pour que rien de renseigné ne reste caché.
function FormSection({ number, title, help, children, collapsible, defaultOpen = true }) {
  const head = (
    <div className="flex items-start gap-3">
      <span className="w-6 h-6 rounded-full bg-[var(--ink-900)] text-white text-xs font-bold flex items-center justify-center shrink-0">{number}</span>
      <div><h3 className="font-semibold text-sm">{title}</h3><p className="text-xs text-[var(--ink-500)] mt-0.5">{help}</p></div>
    </div>
  );
  const body = <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4">{children}</div>;
  if (!collapsible) {
    return <section className="rounded-[8px] border border-[var(--border)] p-4">{head}{body}</section>;
  }
  return (
    <details open={defaultOpen} className="rounded-[8px] border border-[var(--border)] p-4 group">
      <summary className="cursor-pointer list-none flex items-center justify-between gap-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1F6FEB] rounded-[4px]">
        {head}
        <ChevronDown size={16} className="shrink-0 text-[var(--ink-500)] transition-transform group-open:rotate-180" aria-hidden="true" />
      </summary>
      {body}
    </details>
  );
