...
- ✅ **Sprint Polissage visuel institutionnel** (2026-06-22) — frontend uniquement : **armoiries officielles `CIV.png`** (`InstitutionalBrand`), **palette institutionnelle CI** (tokens CSS, **suppression du violet** → or `#C89A2B`), **Sidebar collapsible** (280/76 px, persistance `localStorage`, **un seul item actif**, infobulles), **drawer mobile** off-canvas, espace de validation M&E **dé-violet** (carte blanche/bordure verte), tableau portefeuille collant/large, header nettoyé, composants partagés (`Institutional.jsx`). **Migration des 4 pages référentiels** (PND/Politique/Digital/Alignement) vers les composants partagés (testing_agent `iteration_9.json` 100%). Commit `1a40809`. **testing_agent 100% (iteration_8.json)**. Rapport : `VISUAL_POLISH_METFPA_REPORT.md`.
- ✅ **Fix connexion production** (2026-06-24) — la connexion déployée échouait (HTTP 500) car METFPA utilisait une **2ᵉ base Mongo** (`DB_NAME_metfpa`) non autorisée par la base managée. Correctif : METFPA partage l'**unique base `DB_NAME`** avec **préfixe de collections `metfpa_`** (proxy dans `metfpa/db.py`) — collections legacy intactes, aucune 2ᵉ base. Messages d'erreur login spécifiques (401/403/422/500/réseau). Commit `f09d64c`. **testing_agent 100% (iteration_10.json)**. ⚠️ **Redéploiement requis** pour la production. Rapport : `LOGIN_FAILURE_FIX_REPORT_METFPA.md`.
...
- ✅ **Positionnement institutionnel Preview DIRCAB** (2026-07-10) — masquage des références "formation/training" côté DIRCAB pour présentation Directeur de Cabinet : section **FORMATION supprimée** des sidebars `dircab`, `cabinet_reader`, `me_validator`, `direction_editor` ; le page `ScenarioFormation` déplacée sur route interne **`/_internal/scenario-formation`** gatée `admin only` (via `RoleRoute`), libellée « Guide interne (équipe projet) » sous une rubrique discrète « Ressources internes » réservée à l'admin ; login neutralisé (plus de `admin@metfpa.ci` pré-rempli, placeholder générique `prenom.nom@metfpa.ci`) ; mention « démonstration (formation DIRCAB) » retirée du bas des synthèses `Ordre du jour` ; ajout d'un **badge « Preview DIRCAB »** discret (chip doré institutionnel dans le header, non affiché en mobile). Vérifications Playwright : DIRCAB atterrit sur `/pilotage-directeur`, aucun item Formation/Guide interne visible, `/_internal/scenario-formation` renvoie « Accès refusé » pour DIRCAB et Lecteur Cabinet, admin conserve l'accès complet ; aucune erreur console.
- ✅ **Chargement branche DIRCAB en preview** (2026-07-10) — Fetch manuel de la branche `training-dircab-cockpit` (dépôt privé, token PAT lecture seule, remote temporaire supprimé après usage), déploiement isolé sur l'environnement de prévisualisation **sans merge sur main**. Nouveau rôle **DIRCAB** (`dircab@metfpa.ci`, role=`dircab`) : lecture globale + gestion décisions (create/update/close/arbitrage) sans administration. Backend seed `seed_users()` idempotent, 5 comptes actifs. Seed démo `seed_demo_dircab.py` exécuté avec succès (62 activités re-typées, 6 décisions [DEMO] créées, 5 risques, 2 KPI critiques). Nouveaux modules : `metfpa/validation.py` (workflow M&E state-machine), pages front `OrdreDuJour`, `SuiviHebdo`, `DeclinaisonPeriodique`, `Reporting`, `VueDirections`, `ScenarioFormation`, `ValidationActions`. Login DIRCAB validé en preview via UI (URL: `https://etat-progression.preview.emergentagent.com`).
